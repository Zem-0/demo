...
const Page: React.FC<PageProps> = ({ params }) => {
  ...
  const connect = async () => {
    try {
      ...
      // Use the production backend URL with static survey code and dynamic responder_id
      const wsUrl = `wss://pvbackend-127333528814.us-central1.run.app/survey/voice/350906?responder_id=${responderId}`;
        ...
        <div className="w-full h-svh lg-h-full flex flex-1 items-center justify-center ">
          ...
          <div className="relative w-full h-svh lg:h-auto lg:w-1/5 lg:max-w-[400px] lg:min-w-[350px]">
            ...
            {startPoll ? (
                      ...
                      <div className=" flex flex-row items-center justify-between">
                       ...
                       {/* Displaying voice survey progress and time */}
                              <p className="text-Golden text-exsm font-medium flex items-center justify-center">
                                {Math.round(voiceProgress)}% . {voiceTimeRemaining} min
                              </p>
                          </div>
                        </div>
                ...
                {/* Icon Button Row - Styled to match screenshot */}
                <div className="flex justify-center items-center gap-4 py-4 relative">
                  {/* Leftmost button (custom PNG, replaces Aa) */}
                  <button
                    ref={languageBtnRef}
                    className="w-14 h-14 rounded-full flex items-center justify-center"
                    onClick={() => setShowLanguageDropdown((prev) => !prev)}
                  >
                    <img src="/Frame 1000002608.png" alt="Language" className="w-12 h-12" />
                  </button>
                  {showLanguageDropdown && (
                    <div
                      ref={languageDropdownRef}
                      className="absolute left-0 bottom-full mb-2 w-32 bg-white rounded-lg shadow-lg z-50 py-2"
                  ...
                  {/* Rewind button (custom PNG, left of mic) */}
                  <button
                    onClick={() => sendTextMessage("Skip")}
                    disabled={!isConnected || isRecording}
                    className={`w-14 h-14 rounded-full flex items-center justify-center ${!isConnected || isRecording ? 'opacity-50 cursor-not-allowed' : 'cursor-pointer'}`}
                  >
                    <img src="/Frame 1000002606.png" alt="Rewind" className="w-12 h-12" />
                  </button>

                  {/* Main Record/Pause Button (Mic) */}
                  <button
                    onClick={() => {
                    ...
                    className={`w-16 h-16 rounded-full flex items-center justify-center ${!isConnected ? 'opacity-50 cursor-not-allowed' : 'cursor-pointer'} p-0 border-none bg-transparent shadow-none`}
                    style={{boxShadow: isRecording && !isPaused ? '0 4px 24px 0 rgba(124,58,237,0.15)' : 'none'}}
                  >
                    {isRecording && !isPaused ? (
                      <img src="/Btn (2)d.svg" alt="Mic Button Active" className="w-16 h-16" />
                    ) : (
                      <img src="/Btn (2).svg" alt="Mic Button" className="w-16 h-16" />
                    )}
                  </button>

                  {/* Replay button (custom SVG, right of mic) */}
                  <button
                    onClick={() => {
                      stopRecording();
                    }}
                    disabled={!isRecording && !isPaused}
                    className={`w-14 h-14 rounded-full flex items-center justify-center ${!isRecording && !isPaused ? 'opacity-50 cursor-not-allowed' : 'cursor-pointer'}`}
                  >
                    <img src="/Frame 1000002609.svg" alt="Replay" className="w-12 h-12" />
                  </button>

                  {/* Stop button (custom PNG, rightmost) */}
                  <button
                    onClick={stopRecording}
                    disabled={!isRecording && !isPaused}
                    className={`w-14 h-14 rounded-full flex items-center justify-center bg-[rgba(255,255,255,0.08)] ${!isRecording && !isPaused ? 'opacity-50 cursor-not-allowed' : 'cursor-pointer'}`}
                  >
                    <img src="/Frame 1000002607.png" alt="Stop" className="w-12 h-12" />
                  </button>
                          </div>
